...
def reg() :
    ...
    def process2 (inf) : #log in + sign up for id
            ...
            inf = client.recv(1024)
            inf = inf.decode()
            if inf == 'Password Confirm Failed' : print('Password Confirm Failed')
            else : return True
...
def login ():
    ...
    def process (inf) :
            ...
            inf = client.recv(1024)
            inf = inf.decode()
            if inf == 'Wrong Password' or inf == 'ID Not Found' : print(inf)
            else : 
                return True
